...
| Vente facture | `depot_id` sur la vente = dépôt par défaut ; chaque **ligne** peut surcharger (`vente_lignes.depot_id`) |
| Achat réception | Entrée sur `achats.depot_id` (ou défaut) |
| Ajustement produit | Mouvement sur `depot_id` choisi (ou défaut) |
...
| Retour client | Entrée sur `depot_id` retour, sinon `depot_id` de la **ligne d'origine**, sinon dépôt facture, sinon défaut |
...
À la réception achat : CMUP pondéré séparément sur `prixAchatHt` (HT **après remise ligne** si `remise_pct` > 0) et `frais` (TTC), puis recalcul `plancher`. La **remise facture** (`remise_montant`) n'affecte pas le CMUP.
...
`depot_id` optionnel — dépôt de sortie stock **par défaut** (défaut : dépôt par défaut du PDV). Chaque ligne peut surcharger avec son propre `depot_id`. Stock vérifié **par ligne** sur le dépôt concerné.  
`remise_pct` optionnel sur la vente — remise globale en % appliquée sur le HT total (après remises ligne).  
`mode_vente` : `piece` (gros) \| `detail` (unité détail, si `venteAuDetail`)  
`prix_unitaire` : optionnel (TTC, selon mode)  
`remise_pct` sur chaque ligne — remise en % sur le HT de la ligne (0–100).  
...
**Deux niveaux de remise (vente)** :
- **Ligne** (`lignes[].remise_pct`) : réduit HT → TVA → TTC de la ligne
- **Facture** (`remise_pct` sur la vente) : réduction % sur le HT total après remises ligne ; recalcule TVA/TTC et AIRSI
...
`depot_id` optionnel — change le dépôt par défaut (recalcule les mouvements). Les lignes peuvent aussi définir `depot_id` individuellement.  
...
| Champ | Description |
|-------|-------------|
| `remisePct` | Remise ligne en % sur le HT (0–100) |
| `depotId` | Dépôt de sortie stock pour cette ligne (défaut : `depotId` de la vente) |
| `modeVente` | `piece` (gros) \| `detail` |
| `quantiteStock` | Quantité déduite du stock (unité détail) |
| `plancherLigne` | Plancher TTC unitaire au moment de la vente |
| `marge` | Marge unitaire TTC (`prixUnitaire − plancherLigne`) |
| `airsiPct` | Taux copié depuis `produit.airsiPct` à la création de la ligne |
| `airsiMontant` | Montant AIRSI de la ligne (FCFA) |
| `montantApresAirsi` | `montantTtc` après remise globale + AIRSI |
...
### Champs vente (en-tête) — certification & dépôt

| Champ | Type | Description |
|-------|------|-------------|
| `depotId` | number \| null | Dépôt de sortie par défaut ; les lignes peuvent surcharger |
| `remisePct` | number | Remise globale facture en % (sur HT après remises ligne) |
| `remiseMontant` | number | Montant HT de la remise globale (calculé) |
...
`depot_id` optionnel — dépôt d'**entrée** stock (défaut : `depot_id` de la ligne d'origine, puis dépôt de la facture, puis dépôt par défaut PDV).  
...
{ "produit_id": 3, "quantite": 1, "mode_vente": "piece", "remise_pct": 0, "depot_id": 2 }
```

`depot_id` optionnel — affiche le stock disponible **sur ce dépôt** (`stock_actuel`, `stock_label`).  
Retourne `prix_unitaire`, `depot_id` (résolu), montants ligne (`montant_ht`, `montant_tva`, `montant_ttc`), **AIRSI ligne** (`airsi_pct`, `airsi_montant`, `montant_apres_airsi`), `quantite_stock`, `stock_label`, `plancher`, `marge` (si permissions), `contenance`, `unite`, `unite_gros`, `vente_au_detail`.
...
- `prix_unitaire_ht` et `frais` = par **unité gros** (prix catalogue / dernier achat)
- `remise_pct` = remise ligne en % sur le HT (0–100)

| Champ ligne (`achat_lignes`) | Description |
|------------------------------|-------------|
| `remisePct` | Remise % sur le HT de la ligne |
| `prixUnitaireHt` | Prix unitaire HT gros (avant remise) |
| `frais` | Frais TTC par unité gros |
| `montantHt` | HT ligne après remise ligne |
| `montantTva` | TVA calculée sur HT remisé |
| `montantTtc` | TTC ligne après remise ligne |
| `quantiteRecue` | Quantité reçue (unité gros) |
| `quantiteStock` | Quantité stock (unité détail) |

| Champ achat (en-tête) | Description |
|-----------------------|-------------|
| `remiseMontant` | Remise facture en FCFA (déduite du total TTC) |
| `sousTotal` | Σ `montantTtc` des lignes |
| `totalTtc` | `sousTotal − remiseMontant` |
| `depotId` | Dépôt de réception (défaut = dépôt PDV) |
...
```json
{
  ...
  "lignes": [
    { "produit_id": 3, "quantite": 35, "prix_unitaire_ht": 25500, "frais": 50, "remise_pct": 5 }
  ],
  "remise_montant": 1000
}
```
...
`remise_pct` optionnel par ligne (0–100) — remise sur le HT de la ligne.  
`remise_montant` optionnel sur la facture — remise globale en FCFA déduite du total TTC.

**Deux niveaux de remise (achat)** :
- **Ligne** (`remise_pct`) : réduit HT/TVA/TTC de la ligne ; base CMUP = `prix_ht_après_remise + frais_ht`
- **Facture** (`remise_montant`) : réduction commerciale en FCFA sur le total à payer (n'affecte pas le CMUP stock)
...
```json
{
  ...
  "remise_montant": 2000,
  "lignes": [
    { "produit_id": 3, "quantite": 35, "prix_unitaire_ht": 25500, "frais": 50, "remise_pct": 10 }
  ]
}
```

Seules les **commandes** (`statut: commande`) sont modifiables.

### Ligne-info

```json
// POST /achats/ligne-info
{ "produit_id": 3, "quantite": 35, "prix_unitaire_ht": 25500, "remise_pct": 5 }
```

Retourne prévisualisation : `quantite_stock`, `stock_label`, `stock_label_apres`, `mode_achat`, `unite_quantite`, `frais`, `frais_gros`, `remise_pct`, `prix_ht_apres_remise`, `moyenne_achat_ht_apres`, `montant_ht`, `montant_tva`, `montant_ttc`, etc.

Le champ `frais` est prérempli avec le **frais produit** (modifiable via le paramètre `frais` de la requête).  
`remise_pct` modifie les montants ligne et la prévisualisation CMUP (`prix_ht_apres_remise`, `moyenne_achat_ht_apres`).
...
Met à jour stock (× contenance), CMUP (prix HT **après remise ligne** + frais HT), `dernierPrixAchatHt`, solde fournisseur (TTC ligne après remise ligne).
